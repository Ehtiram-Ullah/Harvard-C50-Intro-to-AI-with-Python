...
class MinesweeperAI():
    ...
    def add_knowledge(self, cell, count):
        ...
        # Gather unknown neighbors and adjust count for known mines
        neighbors = set()
        count_copy = count

        for i in range(cell[0] - 1, cell[0] + 2):
            for j in range(cell[1] - 1, cell[1] + 2):
                # Ignore out-of-bounds cells and the cell itself
                if 0 <= i < self.height and 0 <= j < self.width and (i, j) != cell:
                    if (i, j) in self.mines:
                        count_copy -= 1  # Known mine already accounts for 1 count
                    elif (i, j) not in self.safes:
                        neighbors.add((i, j))  # Only add unknown cells
            
        if len(neighbors) > 0:
            self.knowledge.append(Sentence(neighbors, count_copy))
        ...
        while new_info_found:
            ...
            for sentence in self.knowledge:
                    #  Get the set of known safe cells from this sentence
                    #  For each safe cell in that set, call self.mark_safe(...)
